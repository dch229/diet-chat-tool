...
public class JTableModelLogDisplay extends AbstractTableModel {
  ...
  public void addMessage(Message m){
    if(m.getTimeOnServerOfReceipt()!=null){  
      if (data.size()==0)startTime=m.getTimeOnServerOfReceipt().getTime();
    }  
    data.addElement(m);
  ...
  public boolean isCellEditable(int x, int y){
    ...
    if(y==0){
       ...
       if(m.getTimeOnServerOfReceipt()!=null){
         return ""+(( m.getTimeOnServerOfReceipt().getTime())- startTime );
       }
       return "NO TIMESTAMP";
    }
      ...
      return ""+( m2.getServerTypingOnset()- startTime);
    }
