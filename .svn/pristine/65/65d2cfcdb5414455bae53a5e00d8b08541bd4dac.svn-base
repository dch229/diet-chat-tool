...
public class JKeyloggingChatFrame extends javax.swing.JFrame {
    ...
    private void jTextPane2KeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTextPane2KeyReleased
        // TODO add your handling code here:
         String charreleased = ""+evt.getKeyChar();
         String contentsOfTextArea = this.jTextPane2.getText().replace("\n", "");
         System.out.println("Charreleased:"+charreleased+" contents are:"+contentsOfTextArea);
          kl.klo.saveKeypress("Keyreleased", evt.getWhen(), charreleased, ""+evt.getKeyCode(),contentsOfTextArea);  
         if(evt.getKeyChar()=="\n".charAt(0)) {
             if(contentsOfTextArea.startsWith("/sync")){
                try{  
                 
                  System.out.println("SPLITTING1");
                  String[] s = (String[])StringUtils.split(contentsOfTextArea).toArray();
                   System.out.println("SPLITTING2");
                  System.out.println(s[0]);
                  System.out.println(s[1]);
                  System.out.println(s[2]);
                  System.out.println(s[3]);
                  
                       Integer timeTillStart = Integer.parseInt(s[1]);
                       Integer frequency = Integer.parseInt(s[2]);
                       Integer length = Integer.parseInt(s[3]);
                  
                       Thread.sleep(timeTillStart);
                       AudioSynchronizer.tone(kl.klo,frequency,length);
                       jTextPane2.setText("");
                        return;
                   }catch(Exception e){
                        //jTextPane2.setText("You made an error: try this:\n/sync 500 500 500");
                        //System.exit(-4345);
                        try{
                            //jTextPane2.setText(e.toString());
                            return;
             ...
             try{
              ...
              //String textForMainWindow = this.username+": "+contentsOfTextArea+"\n";
              jTextPane1.getStyledDocument().insertString(jTextPane1.getDocument().getLength(), username+": ", sas);   
                 
                 
              String textForMainWindow = contentsOfTextArea+"\n";
              jTextPane1.getStyledDocument().insertString(jTextPane1.getDocument().getLength(), textForMainWindow, null);   
              jTextPane2.setText("");
             }catch (Exception e){
                 e.printStackTrace();
             }  
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(JKeyloggingChatFrame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
