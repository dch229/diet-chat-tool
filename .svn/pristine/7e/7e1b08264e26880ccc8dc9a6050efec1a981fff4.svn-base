...
public class ParticipantConnection extends Thread {
    ...
    if(Debug.debugGROOP4){
        if(this.particip!=null)Debug.printDBG("ParticipantConnection is ERCEIVINGFROM "+this.particip.getUsername());  
    }
    ...
    if(o instanceof MessageKeypressed|| o instanceof MessageWYSIWYGDocumentSyncFromClientInsert ||
      ...
      try{
        ...
        if(Debug.debugGROOP4){
            if(this.particip!=null )Debug.printDBG("BBBBBBBBBBBBBBBBBB\n");  
        }
    ...
    public boolean verifyLoginDetailsAndActivate() throws Exception {
        ...
        if (o instanceof MessageClientLogon) {
            ...
            String scs = m.getServerClientConsistencyID();
            if(ServerClientConsistency.testForConsistency(scs)){
                ...
                CustomDialog.showDialog("A client is attempting to connect to the server, but the server and the client are not using the same version of the software!\n"
          ...
          try{
            ...
            out.writeObject(m);
              ...
              if(!mostRecentErrorhasBeenFlagged){
                  ...
                  try{
                      //
                      particip.getConversation().saveDataToConversationHistory(particip, "", new Date().getTime(), "Disconnected", "Disconnected");
                  }catch (Exception e){
