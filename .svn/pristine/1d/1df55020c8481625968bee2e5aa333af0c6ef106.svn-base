...
public class CCCBYCMazeGameFakeEndOfTurnCRGeneratorEARLYWHY extends CCCBYCMazeGameFakeEdit{
    ...
    public Sequence getNextSequence_NewLine_By_Same_Speaker(Sequence prior, int sequenceNo, Sequences sS, String sender, MessageCBYCDocChangeFromClient mCDC) {
         ...
         if(state==2||state==3){
            mgcSWFT = new MazeGameCapturesSequenceWaitingForTypingLull(sS,this, sender, mCDC.getTimeOnServerOfReceipt(),  dc.elementString, dc.elementStart, dc.elementFinish) ;
        ...
        if(Debug.showEOFCRSTATES)Conversation.printWSlnLog("Main", sender+" is requesting NEWLINETURN, the state is: "+this.state+"B");
           ...
           Conversation.printWSln("MODOS","(B)MazeGameDefaultSequence:"+sender);
            MazeGameDefaultSequence mgds = new MazeGameDefaultSequence(sS, this ,sender, mCDC.getTimeOnServerOfReceipt(), dc.elementString, dc.elementStart, dc.elementFinish);
   ...
   public Sequence getNextSequence_Edit_By_Different_Speaker(Sequence prior,int sequenceNo,Sequences ss,Participant p,MessageCBYCTypingUnhinderedRequest mCTUR){
        String sendername = prior.getSender();
        if(state==2||state==3){
            mgcSWFT = new MazeGameCapturesSequenceWaitingForTypingLull(sS,this, p.getUsername(), mCTUR.getTimeOnServerOfReceipt(),  mCTUR.getElementString(), mCTUR.getElementStart(), mCTUR.getElementFinish()) ;
