...
public class Game {
   ...
   public synchronized void restartSAMESETJUMBLEDLAYOUT(){
    ...
    public synchronized void shuffleList_BeCareful_TheUIToParticipantsMustBeUpdatedAsWell(){
     ...
     public synchronized void changeFirst(){
        ...
        for(int j=0;j<moves.length;j++){
              try{
                  rtc.cC.getC().saveDataToFile("REARRANGEDSET", "server", "server", new Date().getTime(),new Date().getTime(), new Date().getTime(), "", moves[j].getInfoAsVector());
              }catch (Exception e){
    ...
    private void performSwap(){
        try{
           rtc.cC.getC().saveDataToFile("SETCHANGE: "+swaptype+"--"+swapsize, "server","server" , new Date().getTime(),new Date().getTime(), new Date().getTime(), "", null);
       }catch (Exception e){
           e.printStackTrace();
       }
