...
public abstract class MoveONLY extends Move implements Cloneable{
    ...
    public Participant getP() {
     ...
     public void saveState(Participant originOfMove){
         g.rtc.cC.getC().saveDataToFile("ONLY"+"|"+g.getSet()+"|"+ g.getConsecutiveCorrect(), originOfMove.getParticipantID(), originOfMove.getUsername(),new Date().getTime() ,this.timestampOfReceipt,  this.timestampOfReceipt,
                 "", null);
     }
