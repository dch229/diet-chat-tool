...
public class Dyad {
    ...
    public Dyad(String dyadNo,String dyadNoCorrected) {
    ...
    public void addRowFromSpreadsheet(String uniqid,String participantID, String sender , String dyadno, String dyadnocorrected,int dels, int edits, String text, String descType, long sanity, String turntype, String recipient, String cbyctype){
        ...
        Spreadsheet.jo.appendText("1"+uniqid+"\n");
        ...
        if(contiguouscontributions.size()==0||sender.equalsIgnoreCase("server")){
            ContiguousContribution cc = new ContiguousContribution(uniqid,participantID,sender);
            ...
            ContiguousContribution lastContiguousContribution = (ContiguousContribution)contiguouscontributions.lastElement();
       ...
       public void findTSTandCR(ContiguousContribution response){
            boolean foundCR = true;
            boolean foundTST = false;
            int index = this.contiguouscontributions.indexOf(response);
            for(int i =index-1;i>=0;i--){
                ContiguousContribution cCont = (ContiguousContribution)contiguouscontributions.elementAt(i);
      ...
      public void calculateDistancesFromTSTAndFromRESPONSE(){
           ...
           for(int i=0;i<contiguouscontributions.size()-1;i++){
               ContiguousContribution contigc1 = ( ContiguousContribution)contiguouscontributions.elementAt(i);
      ...
      public void calculateAlignmentScores(){
          ...
          for(int i=0;i<contiguouscontributions.size()-1;i++){
              ContiguousContribution contigc1 = ( ContiguousContribution)contiguouscontributions.elementAt(i);
              ContiguousContribution contigc2 = ( ContiguousContribution)contiguouscontributions.elementAt(i+1);
      ...
      public Vector getRowsForSpreadsheet(){
          ...
          for(int i=0;i<contiguouscontributions.size();i++){
              ContiguousContribution contigc = ( ContiguousContribution)contiguouscontributions.elementAt(i);
              Vector contributions = contigc.contributions;
              
              for(int j=0;j<contributions.size();j++){
                  Contribution contri = (Contribution)contributions.elementAt(j);
