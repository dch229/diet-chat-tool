 ...
 * @author sre
 */
public class AlphabeticalTask3WAY_DEPRECATED_DO_NOT_USE {
   ...
   public AlphabeticalTask3WAY_DEPRECATED_DO_NOT_USE(CCGROOP3SEQ3 cC) {
       this.cC =cC;
   }
   ...
   public void divideAandBMovesBetween3OLD(){
    ...
    public void saveSetToFile(){
        try{
         ...
         cC.getC().saveDataToFile("SELINFO", "server","server",senddate,senddate,senddate,"A: "+a.getUsername()+"  B: "+b.getUsername()+" C: "+c.getUsername(),null);
         cC.getC().saveDataToFile("SELINFO", b.getUsername(), a.getUsername(),senddate, senddate, senddate,"FORB_SENTTO_A"+sAllMovesForBToSelectSentToA , null);
         cC.getC().saveDataToFile("SELINFO", b.getUsername(), c.getUsername(), senddate,senddate, senddate,"FORB_SENTTO_C"+sAllMovesForBToSelectSentToC , null);
         cC.getC().saveDataToFile("SELINFO", a.getUsername(), b.getUsername(), senddate,senddate, senddate,"FORA_SENTTO_B"+sAllMovesForAToSelectSentToB , null);
         cC.getC().saveDataToFile("SELINFO", a.getUsername(), c.getUsername(), senddate,senddate, senddate,"FORA_SENTTO_C"+sAllMovesForAToSelectSentToC, null);
       }catch(Exception e){
            e.printStackTrace();
       }
    }
    ...
    public void processSelection(Participant p,String s){
        ...
        if(s.equalsIgnoreCase("///next")){
            nextSet("Timeout...next set");
             ...
             cC.getC().saveDataToFile("TASK", p.getParticipantID(), p.getUsername(), new Date().getTime(),new Date().getTime(), new Date().getTime(), message, vAdd);
