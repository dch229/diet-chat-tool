...
public class CCFRAG extends DefaultConversationController{
    ...
    public void processChatText(Participant sender,MessageChatTextFromClient mct){    
        ...
        String[] allWords = StringOperations.splitOnlyText(mct.getText());
        for(int i=0;i<allWords.length;i++){
            if(allWords[i].startsWith("a") | allWords[i].startsWith("A")){
                wordToFrag = allWords[i];
                foundFrag=true;
                break;
            }
        }
        ...
        //processingCRSubdialogue =false;
        if(foundFrag & !processingCRSubdialogue){
            ...
            try{Thread.sleep(2500+ 1000*(wordToFrag.length()/avgTypingSpeed));}catch (Exception e){}
            //c.setNewTurnBeingConstructedNotRelayingOldTurnAddingOldTurnToHistory(sender, mct);
            c.sendArtificialTurnFromApparentOriginToRecipient(apparentOrigin,sender,wordToFrag+"?");
            ...
            c.informParticipantBthatParticipantAIsTyping(apparentOrigin,sender);
            if(avgTypingSpeed<=0)avgTypingSpeed = 3;
            try{Thread.sleep(1000+ 1000*(wordToFrag.length()/avgTypingSpeed));}catch (Exception e){}
            c.setNewTurnBeingConstructedNotRelayingOldTurnAddingOldTurnToHistory(sender, mct);
