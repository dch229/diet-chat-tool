 ...
 * @author sre
 */
public class CCPICTURESTIMULI6WEB_DEPRECATED  extends DefaultConversationController implements InterfaceForJCountDown{

    
    String server = "http://homepages.inf.ed.ac.uk/gmills/";
    ...
    public void initializeloadListOfImageNames(){
        //String sUSERDIR = System.getProperty("user.dir")+File.separatorChar+"webserver"+File.separatorChar+sbfolder;
        //File f = new File(sUSERDIR);
        //File[] flist = f.listFiles();
        //CustomDialog.
    ...
    public synchronized String gotoSet(int srch, boolean alternateDM){
        ...
        imgsrcDHTML = "<html><img src='"+this.server+sbfolder+"/"+directorFile.getName()+ "'></img>";
        imgsrcMHTML = "<html><img src='"+this.server+sbfolder+"/"+matcherFile.getName()+ "'></img>";
        ...
        try{
            ...
            wid.changeWebpage(matcher.getUsername(), "<html><img src='"+this.server+sbfolder+"/"+matcherFile.getName() + "'></img>", "", "");
            wid.changeWebpage(director.getUsername(), "<html><img src='"+this.server+sbfolder+"/"+directorFile.getName()+ "'></img>", "", "",timeToDisplayStimulus);
        ...
        c.saveDataToFile("DATA", "server", "server", new Date().getTime(),new Date().getTime(), new Date().getTime(), director.getUsername()+":"+directorFile.getName()+"----"+matcher.getUsername()+"----"+matcherFile.getName(), null);
    ...
    /* String[] options = {"(1) Select faces with buttons","(2) Select with text, (e.g. /1). Perform errorchecking ","(3) No buttons or errorchecking (THIS IS THE PREVIOUS SETUP)"};   
     ...
     CustomDialog.loadDirectories("HELLO", "server");
     System.exit(033);
    ...
    /*     Vector v = new Vector();
      ...
      */  
        ...
        while((!foundmatcher|!founddirector)&searchindex<flist.length){
                    File f = flist[searchindex];
                    if(f.getName().startsWith("d"+numM+"_")){
                       //System.exit(-22);
                        foundmatcher = true;
                        matcherFile = f;
                        matcherSelection = f.getName().substring(f.getName().length()-6,f.getName().length()-4);
                    }
                    if(f.getName().startsWith("d"+numD+"_")){
                        founddirector = true;
                        directorFile = f;
                        directorSelection = f.getName().substring(f.getName().length()-6,f.getName().length()-4);
                    }
        ...
        try{
            ...
            c.changeWebpage(director, "ID1",imgsrcDHTML,"","",timeToDisplayStimulus);
            wid.changeWebpage(matcher.getUsername(), "<html><img src='"+this.server+sbfolder+"/"+matcherFile.getName() + "'></img>", "", "",timeToDisplayStimulus);
            wid.changeWebpage(director.getUsername(), "<html><img src='"+this.server+sbfolder+"/"+directorFile.getName()+ "'></img>", "", "",timeToDisplayStimulus);
            //c.changeWebpageTextAndColour(matcher, "ID1","correct", "red", "white");
            numberOfIncorrectGuessesForCurrentSet =0;
        }
        ...
        c.saveDataToFile("DATA(2D)", "server", "server",new Date().getTime(), new Date().getTime(), new Date().getTime(), director.getUsername()+":"+directorFile.getName()+"----"+matcher.getUsername()+"----"+matcherFile.getName(), null);
    ...
    public synchronized String gotoSet2M(int srch){
        ...
        while((!foundmatcher|!founddirector)&searchindex<flist.length){
                    File f = flist[searchindex];
                    if(f.getName().startsWith("m"+numM+"_")){
                        //System.exit(-22);
                        foundmatcher = true;
                        matcherFile = f;
                        matcherSelection = f.getName().substring(f.getName().length()-6,f.getName().length()-4);
                    }
                    if(f.getName().startsWith("m"+numD+"_")){
                        founddirector = true;
                        directorFile = f;
                        directorSelection = f.getName().substring(f.getName().length()-6,f.getName().length()-4);
                    }
       ...
       // this.matcherNumber=Integer.parseInt(matcherSelection);
        ...
        try{
            ...
            wid.changeWebpage(matcher.getUsername(), "<html><img src='"+this.server+sbfolder+"/"+matcherFile.getName() + "'></img>", "", "");
            wid.changeWebpage(director.getUsername(), "<html><img src='"+this.server+sbfolder+"/"+directorFile.getName()+ "'></img>", "", "");
    ...
    public void processLoop() {
        super.processLoop();
        if(mode==CCPICTURESTIMULI6WEB_DEPRECATED.mode5TRAININGPHASE)c.resetFromIsTypingtoNormalChatAllAllowedParticipants(super.getIsTypingTimeOut());
    ...
    public void processChatText(Participant sender, MessageChatTextFromClient mct) {
      ...
      if(this.doErrorcheckingInTestPhase){  
        if(mode ==CCPICTURESTIMULI6WEB_DEPRECATED.mode3TESTPHASE_MATCHER || mode ==CCPICTURESTIMULI6WEB_DEPRECATED.mode9TESTPHASE_MATCHER){
        ...
        if(mode==CCPICTURESTIMULI6WEB_DEPRECATED.mode5TRAININGPHASE&&mct.getText().startsWith("/")){
        ...
        else if (mode==CCPICTURESTIMULI6WEB_DEPRECATED.mode5TRAININGPHASE){
...
public synchronized void initiateFirstTestPhaseDirector(final int startindex, final int numberoftrials){
    ...
    Thread t = new Thread(){
        public void run(){
        mode=CCPICTURESTIMULI6WEB_DEPRECATED.mode1TESTPHASE_DIRECTOR;
        ...
        mode=CCPICTURESTIMULI6WEB_DEPRECATED.mode2TESTPHASE_POSTDIRECTOR_PREMATCHER;
...
public synchronized void initiateFirstTestPhaseMatcher(final int startindex, final int numberoftrials, final int returnindex){
        ...
        mode=CCPICTURESTIMULI6WEB_DEPRECATED.mode3TESTPHASE_MATCHER;
        ...
        mode = CCPICTURESTIMULI6WEB_DEPRECATED.mode4POSTTESTPHASE;
...
public synchronized void initiateSecondTestPhaseDirector(final int startindex, final int numberoftrials){
    ...
    Thread t = new Thread(){
        public void run(){
        mode=CCPICTURESTIMULI6WEB_DEPRECATED.mode7TESTPHASE_DIRECTOR;
        ...
        mode=CCPICTURESTIMULI6WEB_DEPRECATED.mode8TESTPHASE_POSTDIRECTOR_PREMATCHER;
...
public synchronized void initiateSecondTestPhaseMatcher(final int startindex, final int numberoftrials, final int returnindex){
        Thread t = new Thread(){
        public void run(){
        mode = CCPICTURESTIMULI6WEB_DEPRECATED.mode9TESTPHASE_MATCHER;
        for(int i=startindex;i<startindex+numberoftrials;i++){
                  singleMatcherTrial(i);
                  try { Thread.sleep(pauseBetweenSetsMatcherTrial);} catch (Exception e){ e.printStackTrace();}   
        }
