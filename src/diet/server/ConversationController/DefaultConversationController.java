...
public abstract class DefaultConversationController extends Thread {
    ...
    public void initialize(Conversation c, ExperimentSettings expSettings) {
        ...
        this.c.displayNEWWebpage(p, "ID1", "Stimuli1", "http://www.google.com", 500, 500, false, false);
    ...
    /**
     * This method must be called once the ConversationController subclass is
     * instantiated. This method starts a thread that periodically calls the
     * processLoop() method of the subclass. This is needed to deal with the "is
     * typing" messages that appear in participants' status windows. It is also
     * designed for interventions that are triggered by timing information (As
     * opposed to incoming turns).
     *
     * @param cC
     */
    ...
    public void run() {
        ...
        while (c.isConversationIsActive()) {
            ...
            try {
                if (this.getProcessLoopSleepTime() > 0) {
                    sleep(getProcessLoopSleepTime());
                }
    ...
    /**
     * Returns the collection of parameters constituting the experiment settings
     * associated with this particular experimental setup.
     *
     * @return
     */
    public ExperimentSettings getExpSettingsDEPRECATED() {
        return expSettings;
    }
    ...
    //This is so that it can be called from the BeanShell UI
    ...
    /**
     * This method is invoked by {@link diet.server.Conversation} whenever the
     * cursor position and/or the highlighted text in a participant's text entry
     * window changes .
     *
     * @param sender Participant who changed cursor position/highlighted text
     * @param mWYSIWYGkp message containing information about cursor change /
     * change of highlighted text
     */
    ...
    /**
     * This method is invoked by {@link diet.server.Conversation} whenever a
     * participant using the single window WYSIWYG interface attempts to gain
     * the "conversational floor".
     *
     * @param sender Participant attempting to gain the conversational floor
     * @param mWTUR message describing the attempt to gain floor, including
     * character typed in attempt
     */
    ...
    public void processClientEvent(Participant origin, MessageClientEvent mce){
       // c.saveDataToConversationHistory("", null);
       // c.saveDataToConversationHistory(null, null, null);
        c.saveDataToConversationHistory(origin, "", mce.getTimeOnClientOfCreation().getTime(),"ClientEvent", mce.getNameOfEvent());
    }
    
    
    
    public String getDescriptionForP(Participant p){
        return "unsetinsuperclass";
    }
    ...
    /**
     *
     * @return how long the thread sleeps inbetween calling processLoop() in the
     * subclass
     */
    ...
    /**
     * Sets how long the thread sleeps inbetween calling processLoop() in the
     * subclass.
     *
