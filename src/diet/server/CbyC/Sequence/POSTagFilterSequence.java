 ...
 *         verb and then be in the middle of another (object) noun phrase: So
 *         e.g. : [NNS or NNP or NN][VBT][DT]
 * 
 *         OR
 * 
 *         [DT][NNS or NN][VBT][DT]
 ...
 *         in . . . Also, will only work with subclasses of
 *         CCCBYCAbstractProbeCR, which implement the triggerIntervention()
 *         method, as desired, in that class.
 * 
 */
...
public class POSTagFilterSequence extends Sequence {
	...
	Vector<POSTagRegex> interventionRegexes = new Vector<POSTagRegex>();
	Vector<POSTagRegex> mentionRegexes = null;
	MaxentTagger tagger;
	...
	public void setMentionRegexes(Vector<POSTagRegex> regexes)
	{
		this.mentionRegexes=regexes;
	}
	...
	public POSTagFilterSequence(Sequences sS, CCCBYCDefaultController cC,
		...
		this.misspellings = misspellings;
		this.tagger = tagger;
		this.interventionRegexes = regexes;
		this.turnSoFar = "";
		this.condition = "";
	...
	public String getFragForIntervention() {
		...
		//List<List<HasWord>> sentencesTyped=MaxentTagger
		//		.tokenizeText(new StringReader(turnSoFar));
		
		List<List<HasWord>> sentences = MaxentTagger
				.tokenizeText(new StringReader(turnSpellingCorrected));
		if (sentences.isEmpty())
			return null;
		List<HasWord> lastSentence = sentences.get(sentences.size() - 1);

		List<TaggedWord> lastTaggedSentence = tagger.tagSentence(lastSentence);
		Conversation.printWSln("Main", "last tagged: " + lastTaggedSentence);
					...
					if (afterWait - beforeWait >= this.waitingTimeBeforeInterventionCheckPoint
						...
						if (frag != null) {
							if (this.cC instanceof CCCBYCAbstractProbeCR) {
								((CCCBYCAbstractProbeCR) this.cC)
										.triggerFragSending(sender, frag);
								this.interventionTriggered = true;
							} else {
								Conversation
										.printWSln(
												"ERROR",
												"Tried to intervene. But controller class is wrong. Should be CCCBYCAbstractProbeCR");
							}
						}
	...
	protected String runFiltersAndGetFrag(List<TaggedWord> tagged) {

		for (POSTagRegex p : this.interventionRegexes) {
	...
	protected boolean matches() {
		...
		for (POSTagRegex p : this.interventionRegexes) {
			String tagSequence = this.getTagSequence(lastTaggedSentence,
					p.endInTagOrWord);
			Matcher m = p.regex.matcher(tagSequence);
			if (m.matches()) {
	...
	protected String getTagSequence(List<TaggedWord> lastSent, String endInTag) {
		String result = "";
