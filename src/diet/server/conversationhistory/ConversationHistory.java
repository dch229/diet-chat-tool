...
public class ConversationHistory implements Serializable {
	...
	public synchronized void saveCBYCSequence(Sequence seq){
        ...
        if(seq instanceof MazeGameClarificationRequest || seq instanceof MazeGameAcknowledgement)
            ...
            this.saveIndividualCBYCSequenceMazeGame(-9999,seq.getStartTime().getTime(),seq.getTimeOfLastEnter(),seq.getSender(),seq.getSender(),
            ...
            this.saveIndividualCBYCSequenceMazeGame(-9999,seq.getStartTime().getTime(),seq.getTimeOfLastEnter(),seq.getSender(),seq.getSender(),
                ...
                for(int i=0;i<v.size();i++){
                   ...
                   this.saveIndividualCBYCSequenceMazeGame(-9999,seq.getStartTime().getTime(),seq.getTimeOfLastEnter(),"server",seq.getApparentOriginForRecipient(recipientName),
        ...
        else if(seq instanceof RecordedSequenceFromApparentOrigin)
        {
            ...
            this.saveIndividualCBYCSequence(-9999,rs.getStartTime().getTime(),rs.getTimeOfLastEnter(),rs.getSender(),rs.getApparentSender(),
            ...
            this.saveIndividualCBYCSequence(-9999,seq.getStartTime().getTime(),seq.getTimeOfLastEnter(),seq.getSender(),seq.getSender(),
                sdi.getString(),seq.getAllRecipients(),false,new Vector(),seq.getAllDocChangesFromSender(),0,
                    seq.getElementText(),seq.getElementStartPos(),seq.getElementFinishPos(),seq.getType()+"(AT)",seq.getTimeOfFirstSend(),seq.getTimeOfLastSend(),seq.getFinalText()   );
           ...
           this.saveIndividualCBYCSequence(-9999,seq.getStartTime().getTime(),seq.getTimeOfLastEnter(),seq.getSender(),seq.getSender(),
                ...
                for(int i=0;i<v.size();i++){
                   ...
                   Vector recipients = new Vector();
                   recipients.addElement(recipientName);
                   this.saveIndividualCBYCSequence(-9999,seq.getStartTime().getTime(),seq.getTimeOfLastEnter(),"server",seq.getApparentOriginForRecipient(recipientName),
                   sdiReceived.getString(),recipients,false,new Vector(),seq.getRecipientsDocChanges(recipientName),0,
	...
	long onset, long enter, String senderUsername,
		...
		Turn t = new CBYCTURN(this, timeOfCreationOnClient,onset, enter, sender, apparentSender, text,
        ...
        private synchronized void saveIndividualCBYCSequenceMazeGame(long timeOfCreationOnClient,long onset, long enter, String senderUsername,
			String apparentSenderUsername, String text, Vector recipientsNames,
			boolean wasBlocked, Vector keyPresses, Vector documentUpdates,
			int turnNumber, String eString, int eStart, int eFinish,
			String type, long timeOfFirstSend, long timeOfLastSend,
		...
		Turn t = new CBYCMAZEGAMETURN(this, timeOfCreationOnClient, onset, enter, sender,
	 ...
	 * @param turnNumber
	 */
	public synchronized void saveMessage(String dtype,long timeOfCreationOnClient, long onset, long enter,String senderUsername, String apparentSenderUsername, String text,
			Vector recipientsNames, boolean wasBlocked, Vector keyPresses, Vector documentUpdates, int turnNumber, Object[] priorTurnByOther) {
		if (turns.size() == 0 & setStartTimeOnFirstMessage& this.startTime <-999999998) {
			this.startTime = onset;
		}
		...
		Turn t = new Turn(this, timeOfCreationOnClient,dtype,onset, enter, sender, apparentSender, text,
				recipients, wasBlocked, keyPresses, documentUpdates,
				parsedWords, parseTree, turnNumber,new Vector(),priorTurnByOther);
	...
	public void saveMazeGameMessage(String cvsPREFIX,long timeOfCreationOnClient,long onset, long enter,
			...
			int indivMveNo, Object[] priorTurnByOther) {
		if (turns.size() == 0 & setStartTimeOnFirstMessage& this.startTime <-999999998) {
			this.startTime = onset;
		}
		...
		if(cvsPREFIX==null)cvsPREFIX="";

		// ----------------------------------------------------------------------------

		MAZEGAMETURN t = new MAZEGAMETURN(cvsPREFIX,this,timeOfCreationOnClient, onset, enter, sender,
				apparentSender, text, recipients, wasBlocked, keyPresses,
				documentUpdates, parsedWords, parseTree, turnNumber, mazeNo,
				moveNo, indivMveNo,  priorTurnByOther);
	...
	public TurnMazeGame saveMessageFromCRMazeGame(long timeOfCreationOnClient,long onset, long enter,
			String senderUsername, String apparentSenderUsername, String text,
			Vector recipientsNames, boolean wasBlocked, Vector keyPresses,
			Vector documentUpdates, int turnNumber, String descType,
			String ackCROther,Object[] priorTurnByOther) {
		if (turns.size() == 0 & setStartTimeOnFirstMessage&& this.startTime <-999999998) {
			this.startTime = onset;
		}
		...
		TurnMazeGame t = new TurnMazeGame(this, timeOfCreationOnClient,onset, enter, sender,
				apparentSender, text, recipients, wasBlocked, keyPresses,
				documentUpdates, parsedWords, parseTree, turnNumber, descType,
				ackCROther,priorTurnByOther);
	...
	public ContiguousTurn getPriorContiguousTurnOfConversant(
        ...
        public synchronized void saveDataToFile(String type, long timeOnClient, long onset, long enter,
			String senderID, String senderUsername, String apparentSenderUsername, String text,
			Vector recipientsNames, boolean wasBlocked, Vector keyPresses,
			Vector documentUpdates, int turnNumber, Vector stringData,Object[] priorTurnByOther) {
		...
		// ----------------------------------------------------------------------------
                ...
                Turn t = new DataToBeSaved(this, type, timeOnClient, onset, enter, senderID, senderUsername, apparentSenderUsername, text,
				recipients, wasBlocked, keyPresses, documentUpdates,
				null, null, turnNumber, stringData,priorTurnByOther);
